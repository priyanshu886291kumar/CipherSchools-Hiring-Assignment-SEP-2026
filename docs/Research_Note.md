...
When practicing Data Structures & Algorithms (DSA), the feedback loop is crystal clear: your code either passes the hidden test cases or it fails. 

Low-Level Design (LLD) is completely different. When an engineer sits down to design a **Parking Lot**, an **Elevator System**, or a **Splitwise app**, they face three big questions:
...
Because there are no automated test fixtures that can grade "good Object-Oriented Design," learners are left guessing. They usually end up memorizing static solutions from YouTube or GitHub without understanding *why* specific design choices were made.
...
- **What people do:** Read someone else's Java/C++ repository and try to recreate it from memory.
- **The Problem:** It reinforces the idea that there is only **one golden solution**. If you used the Strategy Pattern instead of the State Pattern, you have no way of knowing whether your choice was reasonable. It teaches memorization rather than design thinking.
...
- **The Problem:** LeetCode only checks input/output correctness. You can pass the test suite with a 400-line monolithic class filled with nested HashMaps and raw integers. It rewards quick algorithmic hacks and completely ignores SOLID principles, encapsulation, and clean abstractions.
...
1. **Feedback Must Cite Real Evidence:** General comments like *"your design is tightly coupled"* aren't helpful. Feedback needs to point directly to the learner's code:  
   > *"Class `ParkingFloor` directly instantiates `HourlyPricingStrategy` on line 12 instead of taking an `IPricingStrategy` interface in its constructor."*
...
3. **Practice Requires an Iteration Loop, Not a One-Time Test:** The entire point of practice is:
   `Attempt #1 ➔ Inspect Weaknesses ➔ Fix Them in Attempt #2 ➔ Measure Improvement`  
...
`Criterion ➔ Score ➔ Observed Evidence (Quote) ➔ Concern ➔ Actionable Suggestion`

### C. Attempt History & Delta Comparison
Every attempt is saved. When a learner submits a second attempt after fixing their code, they can open a **Comparison View** that shows exact score deltas per dimension, confirming that they actually resolved the previous flaws.
